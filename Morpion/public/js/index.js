...
// Submit du formulaire pour créer une room
...
socket.on('play', (enemyPlayer) => {
    if(enemyPlayer.socketId !== player.socketId && !enemyPlayer.turn) {
        const playedCell = document.getElementById(`${enemyPlayer.playedCell}`);
        ...
        SetTurnMessage('alert-info', 'alert-succes', `C'est ton tour de jouer`);
        player.turn = true;
